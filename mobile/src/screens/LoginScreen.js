...
export default function LoginScreen({ navigation }) {
  ...
  const handleLogin = async () => {
    ...
    setError("");
    try {
      const response = await client.post("/login/", { email: email.trim(), password });
      await saveSession(response.data);
      clearWebFocus();
      navigation.replace("Home");
    } catch (requestError) {
      const message = requestError?.response?.data?.detail || "No se pudo iniciar sesión.";
                ...
                <View style={[styles.card, desktop && styles.cardDesktop]}>
                  ...
                  >
                    <View>
                      ...
                      <Text style={styles.primaryText}>{submitting ? "Conectando..." : "Entrar a EcoTracker"}</Text>
                    </View>
...
const styles = StyleSheet.create({
  ...
  error: { color: colors.danger, fontSize: 9, lineHeight: 14, marginTop: 12, textAlign: "center" },
  secureRow: {
    marginTop: 16,
    flexDirection: "row",
    alignItems: "center",
