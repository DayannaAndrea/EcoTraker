...
export default function FoodRegisterScreen({ navigation }) {
  ...
  const addRecord = async () => {
    ...
    setError("");
    try {
      await addFoodRecord({ food, portions });
      setQuantity("1");
    } catch (requestError) {
      setError(requestError?.response?.data?.detail || "No se pudo guardar la comida.");
    } finally {
      setSaving(false);
    }
  };
          ...
          <View style={[styles.shell, desktop && styles.shellDesktop]}>
            ...
            <View style={[styles.columns, desktop && styles.columnsDesktop]}>
              <View style={styles.formCard}>
                ...
                <TouchableOpacity style={styles.save} activeOpacity={0.86} onPress={addRecord} disabled={saving}><Text style={styles.saveText}>{saving ? "Guardando..." : "Guardar comida"}</Text><Text style={styles.saveArrow}>→</Text></TouchableOpacity>
              </View>
