...
export default function RankingScreen({ navigation }) {
    ...
    client.get("/ranking/")
      .then(response => {
        ...
        const normalized = data
          .map((item, index) => ({
            id: String(item.username || index),
            name: item.username || "Usuario",
            footprint: Number(item.weekly_footprint || 0),
            me: Boolean(user?.username && item.username === user.username)
          }))
          .sort((a, b) => a.footprint - b.footprint)
          .map((item, index) => ({ ...item, position: index + 1 }));
        if (active) {
          setRankingData(normalized);
          setSelectedId(normalized.find(item => item.me)?.id || normalized[0]?.id || null);
          setError(null);
        }
      })
      .catch(error => {
        if (active) {
          setRankingData([]);
          setError(error?.response?.status === 401 ? "Tu sesión no está autorizada para consultar el ranking." : "No se pudo cargar el ranking.");
        }
      })
  ...
  const selected = ranking.find(item => item.id === selectedId) || ranking[0] || null;
  const me = ranking.find(item => item.me) || ranking[0] || null;
  const leader = ranking[0] || null;
  const average = ranking.length ? ranking.reduce((sum, item) => sum + item.footprint, 0) / ranking.length : 0;
  const distanceToLeader = me && leader ? Math.max(0, me.footprint - leader.footprint) : 0;
    ...
    return (
      <AnimatedBackground>
        <SafeAreaView style={styles.safe}>
          <View style={[styles.shell, desktop && styles.shellDesktop, styles.loadingState]}>
            <Brand compact />
            <View style={styles.loadingCard}>
              <SectionLabel>CLASIFICACIÓN</SectionLabel>
              <Text style={styles.selectedName}>Cargando ranking...</Text>
              <Text style={styles.progressHint}>Consultando la huella semanal.</Text>
            </View>
          </View>
        </SafeAreaView>
      </AnimatedBackground>
          ...
          <ScrollView contentContainerStyle={styles.scroll}>
            <View style={[styles.shell, desktop && styles.shellDesktop]}>
              <View style={styles.header}>
                <Brand compact />
                <TouchableOpacity activeOpacity={0.82} style={styles.backButton} onPress={() => { clearWebFocus(); navigation.navigate("Home"); }}>
                  <Text style={styles.backArrow}>←</Text><Text style={styles.backText}>INICIO</Text>
                </TouchableOpacity>
              </View>
              <View style={styles.selectedCard}>
                <SectionLabel>CLASIFICACIÓN</SectionLabel>
                <Text style={styles.selectedName}>{error || "No hay datos de ranking todavía."}</Text>
                <Text style={styles.progressHint}>Cuando existan datos semanales y tu sesión esté autorizada, aparecerán aquí.</Text>
              </View>
            </View>
          </ScrollView>
        </SafeAreaView>
          ...
          >
            ...
            <View style={[styles.statsGrid, desktop && styles.statsGridDesktop]}>
              ...
              <View style={styles.statCard}>
                ...
                <Text style={styles.statValue}>{me && leader ? distanceToLeader.toFixed(1) : "-"}</Text>
              ...
              <View style={styles.progressTrack}>
                <View
                  style={[
                    styles.progressFill,
                    {
                      width: `${Math.max(8, Math.min(100, (selected.footprint / Math.max(ranking[ranking.length - 1]?.footprint || 1, 1)) * 100))}%`
                    }
                  ]}
                />
              </View>
            ...
            <View style={styles.listCard}>
              ...
              <View style={styles.rows}>
                {ranking.map((item) => {
                  ...
                  const percentage =
                    (item.footprint / Math.max(ranking[ranking.length - 1]?.footprint || 0, 1)) * 100;
