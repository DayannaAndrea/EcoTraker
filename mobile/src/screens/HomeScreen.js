...
export default function HomeScreen({ navigation }) {
  ...
  const { foodRecords, travelRecords, allRecords, chartRecords, foodTotal, travelTotal, totalImpact, refreshDashboard } = useImpact();
  const { user, logout } = useAuth();
  const [tipRecommendation, setTipRecommendation] = useState(null);

  const chart = useMemo(() => buildImpactChart(chartRecords.length ? chartRecords : allRecords, period), [chartRecords, allRecords, period]);
  ...
  useEffect(() => {
    ...
    const loadTip = async () => {
      try {
        ...
        if (active && data?.tip && data?.category) {
          setTipRecommendation({
            category: data.category,
            impact: data.impact,
            tip: data.tip
          });
        } else if (active) {
          setTipRecommendation(null);
        }
      } catch {
        if (active) setTipRecommendation(null);
      }
    };
          ...
          <View style={[styles.shell, desktop && styles.shellDesktop]}>
            <View style={styles.header}>
              ...
              <View style={styles.headerRight}>
                ...
                <TouchableOpacity style={styles.avatar} activeOpacity={0.8} onPress={async () => { await logout(); clearWebFocus(); navigation.replace("Login"); }}>
            ...
            {tipRecommendation ? <TipCard recommendation={tipRecommendation} /> : null}
            ...
            <View style={styles.footer}>
              <TouchableOpacity activeOpacity={0.7} onPress={async () => { await logout(); clearWebFocus(); navigation.replace("Login"); }}>
